...
impl AnthropicClient {
    ...
    pub async fn send_message(
        ...
        let v = response.json::<serde_json::Value>().await.map_err(ApiError::from)?;
        let mut response = openai_to_anthropic_response(v, request_id.clone()).ok_or(ApiError::MissingApiKey)?;
    ...
    async fn send_raw_request(
        ...
        let req_body = anthropic_to_openai(request);
        
        let request_builder = self
            .http
            .post(&request_url)
            .header("content-type", "application/json");
        let request_builder = self.auth.apply(request_builder);

        let request_builder = request_builder.json(&req_body);
...
mod tests {
    ...
    fn auth_source_applies_headers() {
        // ApiKey-only: bearer auth with the api key
        ...
        // BearerToken-only: bearer auth with the token
        let auth = AuthSource::BearerToken("proxy-token".to_string());
        let request = auth
            .apply(reqwest::Client::new().post("https://example.test"))
            .build()
            .expect("request build");
...
use crate::types::{InputContentBlock, ToolResultContentBlock, OutputContentBlock, Usage};

fn anthropic_to_openai(req: &MessageRequest) -> Value {
    ...
    for msg in &req.messages {
        ...
        for block in &msg.content {
            match block {
                InputContentBlock::Text { text: t } => text.push_str(t),
                ...
                InputContentBlock::ToolResult { tool_use_id, content, .. } => {
                    ...
                    for c in content {
                        if let ToolResultContentBlock::Text { text: t } = c {
                            txt.push_str(t);
                        }
                    }
                    messages.push(json!({
        ...
        if !text.is_empty() || !tool_calls.is_empty() {
            let mut j = json!({"role": role, "content": text});
            if !tool_calls.is_empty() {
                j["tool_calls"] = json!(tool_calls);
            }
            messages.push(j);
        }
    }
    ...
    if req.max_tokens > 0 {
        out["max_tokens"] = json!(req.max_tokens);
    }
    
    if let Some(t_defs) = &req.tools {
        let tools: Vec<Value> = t_defs.iter().map(|t| {
            json!({
                "type": "function",
                "function": {
                    "name": &t.name,
...
fn openai_to_anthropic_response(v: Value, request_id: Option<String>) -> Option<MessageResponse> {
                    ...
                    for t in tc {
                        if let Some(f) = t.get("function") {
                            content.push(OutputContentBlock::ToolUse {
                                id: t.get("id").and_then(|i| i.as_str()).unwrap_or_default().to_string(),
                                name: f.get("name").and_then(|n| n.as_str()).unwrap_or_default().to_string(),
                                input: serde_json::from_str(f.get("arguments").and_then(|a| a.as_str()).unwrap_or("{}")).unwrap_or_else(|_| json!({})),
                            });
                        }
                    }
                }
    ...
    Some(MessageResponse {
        ...
        content,
        model: v.get("model").and_then(|m| m.as_str()).unwrap_or_default().to_string(),
        stop_reason: v.get("choices").and_then(|c| c.as_array()).and_then(|c| c.first()).and_then(|c| c.get("finish_reason")).and_then(|f| f.as_str()).map(|s| s.to_string()),
        stop_sequence: None,
        usage: Usage {
            input_tokens: v.get("usage").and_then(|u| u.get("prompt_tokens")).and_then(|u| u.as_u64()).unwrap_or_default() as u32,
            output_tokens: v.get("usage").and_then(|u| u.get("completion_tokens")).and_then(|u| u.as_u64()).unwrap_or_default() as u32,
