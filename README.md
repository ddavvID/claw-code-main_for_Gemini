...
## 🔄 Gemini API Migration (Rust Crate)

The Rust `api` crate has been migrated from OpenRouter to the **Google Gemini API** using Gemini's [OpenAI-compatible endpoint](https://ai.google.dev/gemini-api/docs/openai). This allows the existing internal type system (`MessageRequest`, `MessageResponse`, `StreamEvent`, etc.) to remain intact while routing requests to Gemini.

### Strategy

Instead of rewriting all internal types to match the native Gemini `generateContent` format, the migration leverages Gemini's **OpenAI Chat Completions compatibility layer**:

1. Internal `MessageRequest` (Anthropic-style) is converted to OpenAI Chat Completions JSON via `anthropic_to_openai()`
2. Requests are sent to `https://generativelanguage.googleapis.com/v1beta/openai/chat/completions`
3. OpenAI-format responses are converted back to internal `MessageResponse` via `openai_to_anthropic_response()`
4. SSE streaming chunks are parsed in OpenAI `delta` format

### Changed Files

| File | Changes |
|------|---------|
| `rust/crates/api/src/client.rs` | Base URL → Gemini OpenAI-compat endpoint; env var → `GEMINI_API_KEY`; added format conversion functions; request path → `/chat/completions` |
| `rust/crates/api/src/sse.rs` | `parse_frame()` rewritten to parse OpenAI SSE chunk format (`choices[].delta.content`) |
| `rust/crates/rusty-claude-cli/src/args.rs` | Default model → `gemini-2.5-flash` |
| `rust/crates/api/tests/client_integration.rs` | Mock responses updated to OpenAI format; path/header assertions updated |
...
| `GEMINI_BASE_URL` | (Optional) Override the API base URL | `https://generativelanguage.googleapis.com/v1beta/openai` |

### Rust CLI Quickstart (Gemini)

```bash
# 1. Set your Gemini API key
export GEMINI_API_KEY="your-gemini-api-key"

# 2. Run with default model (gemini-2.5-flash)
cd rust
cargo run -p rusty-claude-cli

# 3. Or specify a different Gemini model
cargo run -p rusty-claude-cli -- --model gemini-2.5-pro
...
You can use any model available on the Gemini API's OpenAI-compatible endpoint, for example:
